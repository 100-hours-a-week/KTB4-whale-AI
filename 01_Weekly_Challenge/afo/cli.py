...
from afo.core.organizer import organize_files
...
def rename(
    ...
    scan: bool = typer.Option(False, "--scan", help="VirusTotal 보안 스캔 수행 여부 (기본: OFF)"),
):
    """파일명을 지정된 패턴으로 변경"""
    try:
        rename_files(folder, pattern, dry_run, backup, yes, scan)
    except Exception as e:
...
@app.command()
def organize(
    folder: str = typer.Argument(".", help="정리할 폴더 경로"),
    pattern: str = typer.Option("{num:02d}_{title}_{date}", "--pattern", "-p"),
    dry_run: bool = typer.Option(False, "--dry-run", "-n"),
    backup: bool = typer.Option(False, "--backup"),
    yes: bool = typer.Option(False, "--yes", "-y"),
    scan: bool = typer.Option(False, "--scan", help="VirusTotal 보안 스캔 수행 여부 (기본: OFF)"),
):
    """파일명 변경 + 폴더 이동을 한 번에 수행"""
    try:
        asyncio.run(organize_files(folder, pattern, dry_run, backup, yes, scan))
    except Exception as e:
        console.print(f"[red]오류 발생: {e}[/red]")
        raise typer.Exit(1)
